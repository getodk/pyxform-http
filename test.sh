...
if [ "$test_1_actual" != "$test_1_expected" ]; then
  log "test 1 failed: form that converts (with chunked encoding)"
  test_failed="true"
fi
...
if [ "$test_2_actual" != "$test_2_expected" ]; then
  log "test 2 failed: form that fails to convert and returns a pyxform error"
  test_failed="true"
fi
...
if [ "$test_3_actual" != "$test_3_expected" ]; then
  log "test 3 failed: form that converts and also returns pyxform warnings"
  test_failed="true"
fi
...
if [ "$test_5_actual" != "$test_5_expected" ]; then
  log "test 5 failed: form that converts (with external choices)"
  test_failed="true"
fi
...
if [ "$test_6_actual" != "$test_6_expected" ]; then
  log "test 6 failed: form that converts (with no id)"
  test_failed="true"
fi
...
if [ "$test_7_actual" != "$test_7_expected" ]; then
  log "test 7 failed: form that converts (with percent encoded id)"
  test_failed="true"
fi
...
if [ "$test_8_actual" != "$test_8_expected" ]; then
  log "test 8 failed: form that converts (with no id, in XLS format)"
  test_failed="true"
fi
